...
(function (w) {
	...
	(function (a) {
		...
		t("arearange", "area", {
			...
			getGraphPath: function (b) {
				...
				f = [],
				a,
				n = d.area.prototype.getGraphPath,
				c,
				k,
				l;
				l = this.options;
				...
				b = b || this.points;
				for (a = b.length; a--; )
					c = b[a], c.isNull || l.connectEnds || b[a + 1] && !b[a + 1].isNull || f.push({
						plotX: c.plotX,
						plotY: c.plotY,
						doCurve: !1
					}), k = {
			...
			drawDataLabels: function () {
				...
				if (c.enabled || this._hasPointLabels) {
					for (f = e; f--; )
						if (g = b[f])
							r = m ? g.plotHigh < g.plotLow : g.plotHigh > g.plotLow, g.y = g.high, g._plotY = g.plotY, g.plotY = g.plotHigh, a[f] = g.dataLabel, g.dataLabel = g.dataLabelUpper, g.below = r, d ? k || (c.align = r ? "right" : "left") : l || (c.verticalAlign = r ? "top" : "bottom"), c.x = c.xHigh, c.y = c.yHigh;
					n.drawDataLabels && n.drawDataLabels.apply(this, arguments);
					for (f = e; f--; )
						if (g = b[f])
							r = m ? g.plotHigh < g.plotLow : g.plotHigh > g.plotLow, g.dataLabelUpper =
	...
	(function (a) {
		...
		d("waterfall", "column", {
			...
			translate: function () {
				...
				n = this.points;
				d = 0;
				for (a = n.length; d < a; d++)
					c = n[d], m = this.processedYData[d], k = c.shapeArgs, h = (l = w && f.stacks[(this.negStacks && m < u ? "-" : "") + this.stackKey]) ? l[c.x].points[this.index +
			...
			processData: function (b) {
				...
				for (h = 0; h < c; h++)
					r = a[h], d = e && e[h] ? e[h] : {},
				"sum" === r || d.isSum ? a[h] = q(l) : "intermediateSum" === r || d.isIntermediateSum ? a[h] = q(k) : (l += r, k += r),
				m = Math.min(l, m),
				g = Math.max(l, g);
	...
	(function (a) {
		...
		u("polygon", "scatter", {
			...
			type: "polygon",
			getGraphPath: function () {
				for (var a = q.prototype.getGraphPath.call(this), t = a.length + 1; t--; )
					(t === a.length || "M" === a[t]) && 0 < t && a.splice(t, 0, "z");
	...
	(function (a) {
		...
		v.prototype.beforePadding = function () {
			...
			t(G, function (a) {
				...
				if (0 < F)
					for (; c--; )
						d(e[c]) && b.dataMin <= e[c] && e[c] <= b.dataMax && (f = a.radii[c], k = Math.min((e[c] - v) * B - f, k), n = Math.max((e[c] - v) * B + f, n))
			});
			G.length && 0 < F && !this.isLog && (n -= a, B *= (a + k - n) / a, t([["min", "userMin", k], ["max", "userMax", n]], function (a) {
	...
	(function (a) {
		...
		t(d, "translate", function (b) {
			...
			if (a.polar && (this.kdByAngle = a.tooltip && a.tooltip.shared, !this.preventPostTranslate))
				for (b = this.points, a = b.length; a--; )
					this.toXY(b[a])
		});
